...
// compound holding Data, which holds a Version compound with the game's name,
// alongside the other tag types the reader has to skip past to reach it.
func levelDat(version string) []byte {
	var b bytes.Buffer
...
func TestWorldVersion(t *testing.T) {
	...
	os.WriteFile(filepath.Join(dir, "world", "level.dat"), levelDat("1.21.9"), 0o644)
...
func TestWorldVersionCustomLevelName(t *testing.T) {
	...
	os.WriteFile(filepath.Join(dir, "myworld", "level.dat"), levelDat("26.2"), 0o644)
